...
function html(datos) {
  ...
  // Insightly tiene cuota y a veces corta. Si el workflow avisa que no pudo leer
  // el CRM, la fila dice eso en vez de un "0" que se leeria como "no hubo leads".
  const leadsOk = datos.leads_ok !== false;
    ...
    <section class="funnel">
      ...
      <div class="frow">
        <div><div class="fl">Leads cargados en el CRM</div>
             <div class="fsub">${leadsOk
               ? (datos.leads > 0 ? fmtPct(pctLeads) + "% de los visitantes" : "sin leads este mes")
               : "no se pudo leer el CRM este mes"}</div></div>
        <div class="ft"><div class="fb" style="width:${leadsOk ? barra(datos.leads) : 0}%"></div></div>
        <div class="fv">${leadsOk ? num(datos.leads) : "—"}</div>
      </div>
    </section>
